...
//.@Pipe({ name: 'groupBy' })
@Component({
    selector: 'childhealthimmunisationinfo',
    templateUrl: './childhealthimmunisationinfo.component.template.html',    
})

export class ChildHealthImmunisationInfoComponent {
    ...
    DocumentActive;
    isLoading: boolean = false; controllerName = "ChildHealthImmunisationInfo"; 
    ...
    constructor(private apiService: APICallService,private location: Location, private _formBuilder: FormBuilder,
        ...
        this.objChildHealthImmunisationInfo.ControlLoadFormat = ["Default"];
        this.SequenceNo = this.objQeryVal.Id;
        if (this.SequenceNo != 0 && this.SequenceNo != null) {
            this.tblPrimaryKey = this.SequenceNo;
            this.objChildHealthImmunisationInfo.SequenceNo = this.SequenceNo;
        } else {
            this.objChildHealthImmunisationInfo.SequenceNo = 0;
        }

        apiService.post(this.controllerName,"GetDynamicControls",this.objChildHealthImmunisationInfo).then(data => { this.dynamicformcontrol = data; });
        ...
        if (IsUpload) {
            ...
            if (mainFormBuilder.valid && subformbuilder.valid && uploadFormBuilder.valid && dynamicForm != '') {
                ...
                if (this.SequenceNo > 0)
                    type = "update";
                this.objChildHealthImmunisationInfo.DynamicValue = dynamicForm;
                this.objChildHealthImmunisationInfo.ChildId = this.ChildID;
                this.apiService.save(this.controllerName,this.objChildHealthImmunisationInfo, type).then(data => this.Respone(data, type, IsUpload));
            }
        }
    ...
    private Respone(data, type, IsUpload) {
        ...
        if (data.IsError == true) {
            ...
            this.objUserAuditDetailDTO.ChildCarerEmpId = this.ChildID;
            Common.addUserAuditHistoryDetails(this.objUserAuditDetailDTO);
            this._router.navigate(['/pages/child/childhealthimmunisationinfolist/19']);
        }
    }
}