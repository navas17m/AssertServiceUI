...
@Component({
    selector: 'ChildRiskAssessmentFCSignature',
    templateUrl: './riskassessmentfcsignature.component.template.html',
})

export class ChildRiskAssessmentFCSignatureComponents {
    ...
    constructor(private apiService: APICallService, private _formBuilder: FormBuilder,
      ...
      //  this.BindCarer();
        ...
        if(this.UserTypeId==4)
        {
          ...
          //Bind Signature
          this.apiService.get("AgencySignatureCnfg", "GetMappedSignature", 97).then(data => {this.lstAgencySignatureCnfg=data});
    ...
    seTabVisible() {
        ...
        let insChildYPVisible = this.lstCarerSecA.filter(x => x.ControlLoadFormat == 'Child/Young Person');
        if (insChildYPVisible.length > 0) {
            this.ChildYPVisible = false;
        }
        let insFosterVisible = this.lstCarerSecA.filter(x => x.ControlLoadFormat == 'Foster Home');
        if (insFosterVisible.length > 0)
            this.FosterVisible = false;

        let insHealthVisible = this.lstCarerSecA.filter(x => x.ControlLoadFormat == 'Health');
        if (insHealthVisible.length > 0)
            this.HealthVisible = false;

        let insEducationVisible = this.lstCarerSecA.filter(x => x.ControlLoadFormat == 'Education');
        if (insEducationVisible.length > 0)
            this.EducationVisible = false;

        let insContactVisible = this.lstCarerSecA.filter(x => x.ControlLoadFormat == 'Contact Arrangements');
    ...
    private Respone(data, type) {
        ...
        if (data.IsError == true) {
            ...
            this._router.navigate(['/pages/child/riskassessmentlist/4']);
        }
    }
