...
@Component({
    selector: 'childallegationinfodata',
    templateUrl: './childallegationinfodata.component.template.html',
})

export class ChildAllegationInfoDataComponent implements OnInit {
    ...
    showbtnSaveDraft: boolean = true; controllerName = "ChildAllegationInfo";
    ...
    constructor(private apiService: APICallService, private _formBuilder: FormBuilder,
        ...
        this.objAllegationInfo.ChildId = this.ChildID;
    ...
    private Respone(data, type, IsUpload, skipAlert: boolean) {
        ...
        if (data.IsError == true) {
            ...
            if (skipAlert)
            {
                ...
                this.objUserAuditDetailDTO.ChildCarerEmpId = this.ChildID;
                Common.addUserAuditHistoryDetails(this.objUserAuditDetailDTO);
                this._router.navigate(['/pages/child/allegationlist/4']);
            }                
            this.skipAlert = true;
        }
    }
    ...
    ngOnInit() {
        ...
        this.st.newTimer('childalegation180', environment.autoSaveTime);
    ...
    subscribeTimer2() {
        if (this.timer2Id) {
            ...
            this.timer2Id = this.st.subscribe('childalegation180', () => this.timer2callback());
    ...
    ngOnDestroy() {
        this.st.delTimer('childalegation180');
    }

    fnDonloadPDF() {

        window.location.href = this.apiURL + "GenerateChildAllegationsPDF/" + this.ChildCode + "," + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnDonloadWord() {
        window.location.href = this.apiURL + "GenerateChildAllegationsWord/" + this.ChildCode + "," + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnPrint() {
        this.apiService.get("GeneratePDF", "GenerateChildAllegationsPrint", this.ChildCode + "," + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId).then(data => {
            var popupWin;
            // var style = ""; var link = "";
            // var i;
    ...
    fnEmail(form) {
        ...
        if (form.valid) {
            ...
            this.apiService.post("GeneratePDF", "EmailChildAllegations", this.objNotificationDTO).then(data => {
                if (data == true)
                {
