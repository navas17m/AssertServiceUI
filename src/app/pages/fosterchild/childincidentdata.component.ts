...
@Component({
    selector: 'chilsincidentinfodata',
    templateUrl: './childincidentdata.component.template.html',
})

export class ChildIncidentInfoDataComponent implements OnInit {
    ...
    showbtnSaveDraft: boolean = true; controllerName = "ChildIncidentInfo";
    ...
    constructor(private apiService: APICallService, private _formBuilder: FormBuilder,
        ...
        this.objIncidentInfo.ChildId = this.ChildID;
    ...
    private Respone(data, type, IsUpload, skipAlert: boolean) {
        ...
        if (data.IsError == true) {
            ...
            if (skipAlert)
            {
                ...
                this.objUserAuditDetailDTO.ChildCarerEmpId = this.ChildID;
                Common.addUserAuditHistoryDetails(this.objUserAuditDetailDTO);
                this._router.navigate(['/pages/child/incidentlist/3']);
            }
    ...
    ngOnInit() {
        ...
        this.st.newTimer('childincidentinfo180', environment.autoSaveTime);
    ...
    subscribeTimer2() {
        if (this.timer2Id) {
            ...
            this.timer2Id = this.st.subscribe('childincidentinfo180', () => this.timer2callback());
    ...
    ngOnDestroy() {
        this.st.delTimer('childincidentinfo180');
    }
    fnDonloadPDF() {
        window.location.href = this.apiURL + "GenerateChildIncidentPDF/" + this.ChildCode + "," + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnDonloadWord() {
        window.location.href = this.apiURL + "GenerateChildIncidentWord/" + this.ChildCode + "," + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnPrint() {
        this.apiService.get("GeneratePDF", "GenerateChildIncidentPrint", this.ChildCode + "," + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId).then(data => {
            var popupWin;
            // var style = ""; var link = "";
            // var i;
    ...
    fnEmail(form) {
        ...
        if (form.valid) {
            ...
            this.apiService.post("GeneratePDF", "EmailChildIncident", this.objNotificationDTO).then(data => {
                if (data == true)
                {
