...
//.@Pipe({ name: 'groupBy' })
@Component({
        selector: 'childhealthappointmentinfo',
        templateUrl: './childhealthappointmentinfo.component.template.html',
})

export class ChildHealthAppointmentInfoComponent{
    ...
    AppointmentTabActive = "active";
    DocumentActive;
    isLoading: boolean = false; controllerName = "ChildHealthAppointmentInfo"; 
    ...
    constructor(private apiService: APICallService,private location: Location, private _formBuilder: FormBuilder, 
        ...
        this.objChildHealthAppointmentInfo.AgencyProfileId = this.AgencyProfileId;
        this.objChildHealthAppointmentInfo.ChildId = this.ChildID;
        this.SequenceNo = this.objQeryVal.Id;
        if (this.SequenceNo != 0 && this.SequenceNo != null) {
            this.tblPrimaryKey = this.SequenceNo;
            this.objChildHealthAppointmentInfo.SequenceNo = this.SequenceNo;
        } else {
            this.objChildHealthAppointmentInfo.SequenceNo = 0;
        }

        apiService.post(this.controllerName,"GetDynamicControls",this.objChildHealthAppointmentInfo).then(data => { this.dynamicformcontrol = data; });
    ...
    fnAppointmentTab() {
        this.AppointmentTabActive = "active";
        this.DocumentActive = "";
    }
    fnDocumentDetailTab() {
        this.AppointmentTabActive = "";
        ...
        if (IsUpload) {
            ...
            if (mainFormBuilder.valid && subformbuilder.valid && uploadFormBuilder.valid && dynamicForm != '') {
                ...
                if (this.SequenceNo > 0)
                    type = "update";
                this.objChildHealthAppointmentInfo.DynamicValue = dynamicForm;
                this.objChildHealthAppointmentInfo.ChildId = this.ChildID;
                this.apiService.save(this.controllerName,this.objChildHealthAppointmentInfo, type).then(data => this.Respone(data, type, IsUpload));
            }
        }
    ...
    private Respone(data, type, IsUpload) {      
        ...
        if (data.IsError == true) {
            ...
            this.objUserAuditDetailDTO.ChildCarerEmpId = this.ChildID;
            Common.addUserAuditHistoryDetails(this.objUserAuditDetailDTO);
            this._router.navigate(['/pages/child/childhealthappointmentinfolist/19']);
        }
    }

    addNew=false;
    fnAddNewGP() {
        this._router.navigate(['/pages/child/physicianchildmap/19']);
    }
}