...
@Component({
    selector: 'ChildRiskAssessment',
    templateUrl: './riskassessment.component.template.html',
})

export class RiskAssessment implements OnInit {
    ...
    constructor(private apiService: APICallService, private _formBuilder: FormBuilder,
        ...
        if (this.SequenceNo != 0 && this.SequenceNo != null) {
            ...
            this.objRiskAssessmentDTO.ControlLoadFormat = ["Child Details", "Child/Young Person", "Foster Home", "Health", "Education", "Contact Arrangements"]
    ...
    fnBack()
    {
        this._router.navigate(['/pages/child/riskassessmentlist/4/'+this.objQeryVal.apage]);
    }

    seTabVisible() {

        let insChildYPVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Child/Young Person');
        if (insChildYPVisible.length > 0) {
            this.ChildYPVisible = false;
        }
        let insFosterVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Foster Home');
        if (insFosterVisible.length > 0)
            this.FosterVisible = false;

        let insHealthVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Health');
        if (insHealthVisible.length > 0)
            this.HealthVisible = false;

        let insEducationVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Education');
        if (insEducationVisible.length > 0)
            this.EducationVisible = false;

        let insContactVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Contact Arrangements');
        if (insContactVisible.length > 0)
    ...
    clicksubmit(mainFormBuilder,
        ...
        ContactArrangementsDynamic, ContactArrangementsformbuilder,
        ...
        if (this.DocOk && mainFormBuilder.valid &&
            ...
            && ContactArrangementsformbuilder.valid) {
            ...
            ContactArrangementsDynamic.forEach(item => {
                SectionChildDetailsDynamic.push(item);
            });
    ...
    fnDonloadPDF() {
        ...
        window.location.href = this.apiURL + "GenerateRiskAssessmentPDF/" + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId + "," + carerName;
    }
    fnDonloadWord() {
        var carerName = this.carerName.replace('/', '\'');
        window.location.href = this.apiURL + "GenerateRiskAssessmentWord/" + this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId + "," + carerName;
    }
    fnPrint() {
        var carerName = this.carerName.replace('/', '\'');
        this.apiService.get("GeneratePDF", "GenerateRiskAssessmentPrint", this.ChildID + "," + this.SequenceNo + "," + this.AgencyProfileId + "," + carerName).then(data => {
            var popupWin;
            // var style = ""; var link = "";
            // var i;
    ...
    fnEmail(form) {
        ...
        if (form.valid) {
            ...
            this.apiService.post("GeneratePDF", "EmailRiskAssessment", this.objNotificationDTO).then(data => {
                if (data == true)
