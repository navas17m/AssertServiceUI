...
declare var $: any;
@Component({
    selector: 'AnnualReviewRainbow',
    templateUrl: './annualreviewrainbowdata.component.template.html',
    styles: [`[required]  {
...
export class AnnualReviewRainbowDataComponet {
   ...
   // @ViewChild('ApprovalRecommendation',{static:false}) childApprovalRecommendation;
    ...
    constructor(private _formBuilder: FormBuilder,
        ...
        if (Common.GetSession("SaveAsDraft") == "N") {
            ...
            this.objAnnualReviewgetDTO.ControlLoadFormat = ["Page1"];
    ...
    fnBack()
    { this._router.navigate(['/pages/fostercarer/annualreviewlistrainbow/3/'+this.objQeryVal.apage]);
    }
    ...
    fnSaveDraft(form, p1Form, p1formbuilder, IsUpload, uploadFormBuilder) {
        // p2Form.forEach(item => {
        //     p1Form.push(item);
        // });
    ...
    DocOk = true;
    SubmitAnnualReview(form, p1Form, p1formbuilder,
    ...
    private Respone(data, type, IsUpload, skipAlert) {
        ...
        if (data.IsError == true) {
            ...
            this.IsNewOrSubmited = 2;
            let tempdynamicformcontrol = deepCopy<any>(this.dynamicformcontrol);
            this.dynamicformcontrolOG = deepCopy<any>(tempdynamicformcontrol);
    ...
    //PDF
    fnDonloadPDF() {
        window.location.href = this.apiURL + "GenerateCarerAnnualReviewRainbowPDF/" + this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnDonloadWord() {
        window.location.href = this.apiURL + "GenerateCarerAnnualReviewRainbowWord/" + this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnPrint() {
        this.apiService.get("GeneratePDF", "GenerateCarerAnnualReviewRainbowPrint", this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId).then(data => {
            var popupWin;
            var style = ""; var link = "";
            var i;
    ...
    fnEmail(form) {
        ...
        if (form.valid) {
            ...
            this.apiService.post("GeneratePDF", "EmailCarerAnnualReviewRainbow", this.objNotificationDTO).then(data => {
                if (data == true){
