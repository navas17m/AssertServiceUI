...
declare var $: any;
@Component({
    selector: 'annualreviewnurturefcsignature',
    templateUrl: './annualreview-nurture-fcsignature.component.templte.html',
    styles: [`[required]  {
...
export class CarerAnnualReviewNurtureFCSignatureComponet {
  ...
  //  @ViewChild('ApprovalRecommendation') childApprovalRecommendation;
    ...
    constructor(private _formBuilder: FormBuilder,
        ...
        if (Common.GetSession("SelectedCarerProfile") != null) {
         ...
         //Bind Signature
         this.apiService.get("AgencySignatureCnfg", "GetMappedSignature", 408).then(data => {this.lstAgencySignatureCnfg=data});
    ...
    AgencySignatureCnfgChange(id:any) {
    ...
    fnChildChange(values) {
      ...
      LoadAlreadyChildPlacementInfo(data) {
        ...
        //    console.log(data);
        if (data != null) {
            data= data.filter(x=>x.FieldName!='CreatedUserId'&&
            x.FieldName!='CreatedDate' &&
            x.FieldName!='UpdatedUserId' && x.FieldName!='UpdatedDate')
            data.forEach(item => {
                this.objPlacementInfoList = [];
                item.filter(x=>x.FieldName!='CreatedUserId'&&
                x.FieldName!='CreatedDate' &&
                x.FieldName!='UpdatedUserId' && x.FieldName!='UpdatedDate').forEach(subItem => {
