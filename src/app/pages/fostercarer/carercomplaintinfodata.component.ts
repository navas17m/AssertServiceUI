...
@Component({
    selector: 'carercomplaintinfodata',
    templateUrl: './carercomplaintinfodata.component.template.html',
})

export class CarerComplaintInfoDataComponent implements OnInit {
    ...
    showbtnSaveDraft: boolean = true; controllerName = "CarerComplaintsInfo";
    ...
    constructor(private apiService: APICallService, private _formBuilder: FormBuilder,
        ...
        this.objComplaintInfo.CarerParentId = this.CarerParentId;
    ...
    private Respone(data, type, IsUpload, skipAlert: boolean) {
        ...
        if (data.IsError == true) {
            ...
            if (skipAlert){
              ...
              this.objUserAuditDetailDTO.ChildCarerEmpId = this.CarerParentId;
              Common.addUserAuditHistoryDetails(this.objUserAuditDetailDTO);
              this._router.navigate(['/pages/fostercarer/complaintslist/3']);
            }
            this.skipAlert = true;
            this.objComplaintInfo.IsSubmitted=true;
        }
    }
    ...
    ngOnInit() {
        ...
        this.st.newTimer('complaintsinfo180', environment.autoSaveTime);
    ...
    subscribeTimer2() {
        if (this.timer2Id) {
            ...
            this.timer2Id = this.st.subscribe('complaintsinfo180', () => this.timer2callback());
    ...
    ngOnDestroy() {
        this.st.delTimer('complaintsinfo180');
    }

    fnDonloadPDF() {
        window.location.href = this.apiURL + "GenerateCarerComplaintsPDF/" + this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnDonloadWord() {
        window.location.href = this.apiURL + "GenerateCarerComplaintsWord/" + this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnPrint() {
        this.apiService.get("GeneratePDF", "GenerateCarerComplaintsPrint", this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId).then(data => {
            var popupWin;
            // var style = ""; var link = "";
            // var i;
    ...
    fnEmail(form) {
        ...
        if (form.valid) {
            ...
            this.apiService.post("GeneratePDF", "EmailCarerComplaints", this.objNotificationDTO).then(data => {
                if (data == true){
