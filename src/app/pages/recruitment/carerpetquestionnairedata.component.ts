...
@Component({
    selector: 'carerpetquestionnairedata',
    templateUrl: './carerpetquestionnairedata.component.template.html',
})

export class CarerPetQuestionnaireData {
    ...
    constructor(private _formBuilder: FormBuilder,
        ...
        this.objCarerPetQuestionnaireDTO.AgencyProfileId = this.AgencyProfileId;
        this.objCarerPetQuestionnaireDTO.CarerParentId = this.CarerParentId;
        this.objCarerPetQuestionnaireDTO.ControlLoadFormat = ["Pet Questionnaire A", "Pet Questionnaire B", "Pet Questionnaire C", "Pet Questionnaire D"];
        ...
        if (this.SequenceNo != 0 && this.SequenceNo != null) {
            this.objCarerPetQuestionnaireDTO.SequenceNo = this.SequenceNo;
        } else {
            this.objCarerPetQuestionnaireDTO.SequenceNo = 0;
            //  cdlServics.getByFormCnfgId(this.objCarerPetQuestionnaireDTO).then(data => { this.dynamicformcontrol = data; });
            this.apiService.post(this.controllerName, "GetDynamicControls", this.objCarerPetQuestionnaireDTO).then(data => {
        ...
        if (Common.GetSession("SaveAsDraft") == "N") {
            ...
            this.objCarerPetQuestionnaireDTO.SequenceNo = this.objQeryVal.Id;
            this.objCarerPetQuestionnaireDTO.AgencyProfileId = this.AgencyProfileId;
            this.objCarerPetQuestionnaireDTO.CarerParentId = this.CarerParentId;
            //cdlServics.getByFormCnfgId(this.objCarerPetQuestionnaireDTO).then(data => {
            //    this.dynamicformcontrol = data;
            //});
    ...
    seTabVisible() {
        let PageBVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Pet Questionnaire B');
        if (PageBVisible.length > 0)
            this.PageBVisible = true;

        let PageCVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Pet Questionnaire C');
        if (PageCVisible.length > 0)
            this.PageCVisible = true;

        let PageDVisible = this.dynamicformcontrol.filter(x => x.ControlLoadFormat == 'Pet Questionnaire D');
        if (PageDVisible.length > 0)
            this.PageDVisible = true;
    }
    ...
    clicksubmit(mainFormBuilder, SectionAdynamicValue, SectionAdynamicForm, SectionBdynamicValue,
        SectionBdynamicForm, SectionCdynamicValue, SectionCdynamicForm, SectionDdynamicValue, SectionDdynamicForm
        ...
        if (mainFormBuilder.valid && SectionAdynamicForm.valid && SectionBdynamicForm.valid && SectionCdynamicForm.valid
            && SectionDdynamicForm.valid && this.DocOk) {
            ...
            if (this.isDirty) {
                ...
                this.isLoading = true;
                this.objCarerPetQuestionnaireDTO.DynamicValue = SectionAdynamicValue;
                this.objCarerPetQuestionnaireDTO.CarerParentId = this.CarerParentId;
                // this.cdlServics.post(this.objCarerPetQuestionnaireDTO, type).then(data => this.Respone(data, type, IsUpload, this.skipAlert));
                this.apiService.save(this.controllerName, this.objCarerPetQuestionnaireDTO, type).then(data => this.Respone(data, type, IsUpload, this.skipAlert));
            }
            else {
            ...
            if (skipAlert) {
              ...
              Common.addUserAuditHistoryDetails(this.objUserAuditDetailDTO);
                if (this.objQeryVal.mid == 13)
                    this._router.navigate(['/pages/recruitment/carerpetquestionnairelist/13']);
                else
                    this._router.navigate(['/pages/fostercarer/carerpetquestionnairelist/3']);
            }
            this.skipAlert = true;
        }
    }
    ...
    fnSaveDraft(SectionAdynamicValue, SectionBdynamicValue, SectionCdynamicValue, SectionDdynamicValue, IsUpload, uploadFormBuilder) {
    ...
    ngOnInit() {
        ...
        this.st.newTimer('carerpet180', environment.autoSaveTime);
    ...
    subscribeTimer2() {
        if (this.timer2Id) {
            ...
            this.timer2Id = this.st.subscribe('carerpet180', () => this.timer2callback());
    ...
    ngOnDestroy() {
        this.st.delTimer('carerpet180');
    }


    fnDonloadPDF() {
        window.location.href = this.apiURL + "GenerateCarerPetQuestionnairePDF/" + this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnDonloadWord() {
        window.location.href = this.apiURL + "GenerateCarerPetQuestionnaireWord/" + this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId;
    ...
    fnPrint() {
        this.apiService.get("GeneratePDF", "GenerateCarerPetQuestionnairePrint", this.CarerCode + "," + this.CarerParentId + "," + this.SequenceNo + "," + this.AgencyProfileId).then(data => {
            var popupWin;
            // var style = ""; var link = "";
            // var i;
    ...
    fnEmail(form) {
        ...
        if (form.valid) {
            ...
            this.apiService.post("GeneratePDF", "EmailCarerPetQuestionnaire", this.objNotificationDTO).then(data => {
                if (data == true){
